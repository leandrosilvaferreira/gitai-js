...
- [Type-aware ESLint outside src/](architecture-eslint-type-aware-outside-src.md) — adding a dir to the lint glob checks nothing type-aware; rules are scoped by `files:` and `projectService` only finds the root tsconfig.
